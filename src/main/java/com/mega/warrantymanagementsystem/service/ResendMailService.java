...
public class ResendMailService {
    ...
    public void sendResetPasswordMail(String toEmail, String fullName, String resetUrl) {
        ...
        try {
            ...
            body.put("from", "onboarding@resend.dev"); // email test Resend
            body.put("to", List.of(toEmail));
            body.put("subject", "Reset your password");

            // HTML dài nhưng nối từng dòng, chắc chắn serialize được
            String html = "<!doctype html>"
                    + "<html lang='en'>"
                    + "<head>"
                    + "<meta charset='utf-8'>"
                    + "<title>Password Reset Request – Project Name</title>"
                    + "<style>.btn { background:#007bff;color:#fff;padding:12px 18px;border-radius:6px;text-decoration:none;font-weight:700;display:inline-block;}</style>"
                    + "</head>"
            ...
            // serialize bằng ObjectMapper để JSON đúng chuẩn
